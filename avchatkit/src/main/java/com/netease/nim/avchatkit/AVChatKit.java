...
public class AVChatKit {
    ...
    private static ICallUtil iCallUtil;
    ...
    /**
     * 音视频通话呼叫
     * @param iCallUtil 音视频通话呼叫接口
     */
    public static void setiCallUtil(ICallUtil iCallUtil) {
        AVChatKit.iCallUtil = iCallUtil;
    }
     ...
     * @param context      上下文
     * @param config       呼叫方信息
     * @param displayName  呼叫方显示名称
     * @param source       发起呼叫的来源，参考AVChatActivityEx.FROM_INTERNAL/FROM_BROADCASTRECEIVER
     */
    public static void incomingCall(Context context,
                                    AVChatData config,
                                    String displayName,
                                    int source) {
        if (AVChatKit.iCallUtil != null)
            AVChatKit.iCallUtil.incomingCall(context, config, displayName, source);
    }
     ...
     * @param source        发起呼叫的来源，参考AVChatActivityEx.FROM_INTERNAL/FROM_BROADCASTRECEIVER
     */
    public static void outgoingCall(Context context,
                                    String account,
                                    String displayName,
                                    int callType,
                                    int source) {
        if (AVChatKit.iCallUtil != null)
            AVChatKit.iCallUtil.outgoingCall(context, account, displayName, callType, source);
    }

    /**
     * 发起群组音视频通话呼叫
     * @param context   上下文
     * @param roomName  音视频通话房间名称
     */
    public static void outgoingTeamCall(Context context, String roomName) {
        if (AVChatKit.iCallUtil != null)
            AVChatKit.iCallUtil.outgoingTeamCall(context, roomName);
    }
    ...
    public static void startAVChatSettings(Context context) {
        if (AVChatKit.iCallUtil != null)
            AVChatKit.iCallUtil.startSettings(context);
    }
    ...
    private static Observer<AVChatData> inComingCallObserver = new Observer<AVChatData>() {
        ...
        public void onEvent(final AVChatData data) {
            ...
            AVChatProfile.getInstance().launchActivity(data, userInfoProvider.getUserDisplayName(data.getAccount()), FROM_BROADCASTRECEIVER);
        }
    };
